document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelector('form').addEventListener('submit', event => {
        ...
        if (!(regex.test(document.querySelector('input#email').value))) {
            event.currentTarget.classList.add('err');
            event.preventDefault();
        }
    });
    // remove err
    document.querySelector('form').addEventListener('input', event => event.currentTarget.classList.remove('err'));
    ...
    document.querySelectorAll('svg[data-wavy]').forEach(wavy => {
        ...
        wavy.style.cssText = `
            ...
            fill: ${wavy.dataset.wavyColor};
        `;

        // new wavy
        new Wavy(wavy, 4, 0.01, 0.025);
    });
});
...
class Point {
    constructor(x, max, minSpeed, maxSpeed) {
        this.height = this.random(max / 2, max); // rand amp
